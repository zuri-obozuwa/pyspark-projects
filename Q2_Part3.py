...
print("looking at categorical vectors in full:")
insurance_imp.select("catfeatures").show(10, truncate=False)
...
print("looking at final vectors in full:")
insurance.select("finalvector").show(10, truncate=False)
...
#Checking its same split of data as part 2
trainingData.select("Row_ID", "binaryclaim").show(150)

####3a: Binary classifier

from pyspark.ml.classification import RandomForestClassifier
from pyspark.ml.evaluation import MulticlassClassificationEvaluator

rfc = RandomForestClassifier(labelCol="binaryclaim", featuresCol="finalvector", maxDepth=10)

accuracyEval = MulticlassClassificationEvaluator(labelCol="binaryclaim", predictionCol="prediction", metricName="accuracy")

rfcmodel = rfc.fit(trainingData)
rfc_pred = rfcmodel.transform(testData)
...
MSEevaluator = RegressionEvaluator(labelCol='Claim_Amount', predictionCol="combo_prediction", metricName="mse")
MAEevaluator = RegressionEvaluator(labelCol='Claim_Amount', predictionCol="combo_prediction", metricName="mae")


mse = MSEevaluator.evaluate(combo_pred)
mae = MAEevaluator.evaluate(combo_pred)
